...
pub async fn process_reindex_queue(state: Arc<AppState>, mut rx: mpsc::Receiver<WatchEvent>) {
    while let Some(event) = rx.recv().await {
        match &event {
            WatchEvent::RulesetChanged { path } => {
                info!(path = %path.display(), "Processing ruleset watch event");
            }
            WatchEvent::CategoryMapChanged => {
                info!("Processing ruleset map watch event");
            }
        }
